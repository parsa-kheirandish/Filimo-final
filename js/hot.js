...
const hotList = document.querySelector(".hotProducts");
let portraiteColumn = 8;
let hotRow = 1;
  // hot list generate------------->
  const fetchMediasHot = async () => {
  let data = await fetch(`${dataBase}`);
  let res = await data.json();
  const filteredHot = res.filter((val) => val.hot);
  filteredHot.forEach((val, index) => {
    if (index < (portraiteColumn*hotRow)){
      hotList.innerHTML += `
      <div class="hotProduct h-[80vw] sm:h-[33vw] lg:h-[17vw] cursor-pointer overflow-hidden rounded-[10px]">
          <div class="hotImage h-[75vw] sm:h-[30vw] lg:h-[15vw] flex justify-center group relative ">
          <!-- -------------اطلاعات عکس-------------->    
