...
const awesomeList = document.querySelector(".awesomeProducts");

// awesome list generate------------->
const fetchMediasAwesome = async () => {
  let data = await fetch(`${dataBase}`);
  let res = await data.json();
  const filteredAwesome = res.filter((val) => val.awesome);
  filteredAwesome.forEach((val, index) => {
    if (index < (portraiteColumn*awesomeRow)){
      awesomeList.innerHTML += `
      <div class="awesomeProduct h-[80vw] sm:h-[33vw] lg:h-[17vw] cursor-pointer overflow-hidden rounded-[10px]">
          <div class="awesomeImage h-[75vw] sm:h-[30vw] lg:h-[15vw] flex justify-center group relative ">
          <!-- -------------اطلاعات عکس-------------->    
...
export default fetchMediasAwesome;