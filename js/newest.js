const dataBase = "https://parsa-kheirandish.github.io/Filimo-API/db.json"
  ...
  let newRow = 1;
  const newList = document.querySelector(".newestProducts");
  // new list generate------------->
  const fetchMediasNewest = async () => {
  let data = await fetch(`${dataBase}`);
  let res = await data.json();
  const filteredNew = res.filter((val) => val.new);
  filteredNew.forEach((val, index) => {
   if (index < (portraiteColumn*newRow)){
     newList.innerHTML += `
     <div class="newestProduct h-[80vw] sm:h-[33vw] lg:h-[17vw] cursor-pointer overflow-hidden rounded-[10px]">
         <div class="newestImage h-[75vw] sm:h-[30vw] lg:h-[15vw] flex justify-center group relative ">
         <!-- -------------اطلاعات عکس-------------->    
  ...
  export default fetchMediasNewest