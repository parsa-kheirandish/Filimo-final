...
const popularList = document.querySelector(".PopularProducts");
let portraiteColumn = 8;
let popularRow = 1;
    // popular list generate------------->
    const fetchMediasPopular = async () => {
  let data = await fetch(`${dataBase}`);
  let res = await data.json();
  const filteredPopular = res.filter((val) => val.popular);
  filteredPopular.forEach((val, index) => {
    if (index < (portraiteColumn*popularRow)){
      popularList.innerHTML += `
      <div class="popularProduct h-[80vw] sm:h-[33vw] lg:h-[17vw] cursor-pointer overflow-hidden rounded-[10px]">
          <div class="popularImage h-[75vw] sm:h-[30vw] lg:h-[15vw] flex justify-center group relative ">
          <!-- -------------اطلاعات عکس-------------->    
  ...
  export default fetchMediasPopular;